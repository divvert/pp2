...
namespace Task3
{
    class Program
    {
        static void PrintSpaces(int lvl)//функция для отступов в новой директории
        {
            for (int i = 0; i < lvl; i++)
                Console.Write(" ");
        }
        static void DirectoryTree(DirectoryInfo dir, int lvl)//функиця для просмотра всех файлов и директорий
        {
        ...
        {
            DirectoryInfo path = new DirectoryInfo(@"C:\Users\даяна\Desktop\pp2");//указываем путь директории которую хотим просмотреть
